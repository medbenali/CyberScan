...
from scapy_local.data import KnowledgeBase
from scapy_local.config import conf
from scapy_local.layers.inet import IP, TCP, TCPOptions
from scapy_local.packet import NoPayload
...
Some specifications of the p0f.fp file are not (yet) implemented."""
    ...
    if pers[0] == '*':
        ...
        pkt.payload.window = [x for x in options if x[0] == 'MSS'][0][1] * int(pers[0][1:])
    else:
