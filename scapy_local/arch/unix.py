...
from scapy_local.error import warning
import scapy_local.config
import scapy_local.utils
import scapy_local.utils6
import scapy_local.arch

scapy_local.config.conf.use_pcap = 1
scapy_local.config.conf.use_dnet = 1
...
def read_routes():
    if scapy_local.arch.SOLARIS:
        f=os.popen("netstat -rvn") # -f inet
    elif scapy_local.arch.FREEBSD:
    ...
    for l in f.readlines():
        ...
        if dest == "default":
            ...
            if scapy_local.arch.SOLARIS:
                netmask = scapy_local.utils.atol(mask)
            elif "/" in dest:
                dest,netmask = dest.split("/")
                netmask = scapy_local.utils.itom(int(netmask))
            else:
                netmask = scapy_local.utils.itom((dest.count(".") + 1) * 8)
            dest += ".0"*(3-dest.count("."))
            dest = scapy_local.utils.atol(dest)
        if not "G" in flg:
            gw = '0.0.0.0'
        if netif is not None:
            ifaddr = scapy_local.arch.get_if_addr(netif)
    ...
    for dest,netmask,gw in pending_if:
        gw_l = scapy_local.utils.atol(gw)
...
def in6_getifaddr():
    ...
    for int in i:
        ...
        for a in v6:
            ...
            xx = str(a).split('/')[0]
            addr = scapy_local.utils6.in6_ptop(xx)

            scope = scapy_local.utils6.in6_getscope(addr)
...
def read_routes6():
    ...
    for l in f.readlines():
        ...
        if scapy_local.arch.NETBSD or scapy_local.arch.OPENBSD:
            lspl = l.split()
            d,nh,fl = lspl[:3]
        ...
        if '%' in nh:
            nh,dev = nh.split('%')
        if scapy_local.arch.LOOPBACK_NAME in dev:
            cset = ['::1']
            nh = '::'
        else:
            devaddrs = [x for x in lifaddr if x[2] == dev]
            cset = scapy_local.utils6.construct_source_candidate_set(d, dp, devaddrs, scapy_local.arch.LOOPBACK_NAME)
