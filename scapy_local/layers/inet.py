...
from scapy_local.utils import checksum
from scapy_local.layers.l2 import *
from scapy_local.config import conf
from scapy_local.fields import *
from scapy_local.packet import *
from scapy_local.volatile import *
from scapy_local.sendrecv import sr,sr1,srp1
from scapy_local.plist import PacketList,SndRcvList
from scapy_local.automaton import Automaton,ATMT

import scapy_local.as_resolvers
...
class TCP(Packet):
    ...
    def post_build(self, p, pay):
        ...
        if self.chksum is None:
            ...
            elif conf.ipv6_enabled and isinstance(self.underlayer, scapy_local.layers.inet6.IPv6) or isinstance(self.underlayer, scapy_local.layers.inet6._IPv6ExtHdr):
                ck = scapy_local.layers.inet6.in6_chksum(socket.IPPROTO_TCP, self.underlayer, p)
                p = p[:16]+struct.pack("!H", ck)+p[18:]
            else:
    ...
    def mysummary(self):
        ...
        elif conf.ipv6_enabled and isinstance(self.underlayer, scapy_local.layers.inet6.IPv6):
...
class UDP(Packet):
    ...
    def post_build(self, p, pay):
        ...
        if self.chksum is None:
            ...
            elif isinstance(self.underlayer, scapy_local.layers.inet6.IPv6) or isinstance(self.underlayer, scapy_local.layers.inet6._IPv6ExtHdr):
                ck = scapy_local.layers.inet6.in6_chksum(socket.IPPROTO_UDP, self.underlayer, p)
                p = p[:6]+struct.pack("!H", ck)+p[8:]
            else:
                warning("No IP underlayer to compute checksum. Leaving null.")
        return p
    ...
    def mysummary(self):
        ...
        elif isinstance(self.underlayer, scapy_local.layers.inet6.IPv6):
...
PacketList.timeskew_graph = types.instancemethod(_packetlist_timeskew_graph, None, PacketList)
...
#                loctrace.append((ip,loc)) # no labels yet
    ...
    def make_graph(self,ASres=None,padding=0):
        ...
        ports_done = {}
        for s,r in self.res:
            r = r.getlayer(IP) or (conf.ipv6_enabled and r[scapy_local.layers.inet6.IPv6]) or r
            s = s.getlayer(IP) or (conf.ipv6_enabled and s[scapy_local.layers.inet6.IPv6]) or s
            ips[r.src] = None
            if TCP in s:
                ...
                trace_id = (s.src,s.dst,s.proto,0)
            trace = rt.get(trace_id,{})
            ttl = conf.ipv6_enabled and scapy_local.layers.inet6.IPv6 in s and s.hlim or s.ttl
            if not (ICMP in r and r[ICMP].type == 11) and not (conf.ipv6_enabled and scapy_local.layers.inet6.IPv6 in r and ICMPv6TimeExceeded in r):
                if trace_id in ports_done:
                    continue
                ports_done[trace_id] = None
                p = ports.get(r.src,[])
...
if conf.ipv6_enabled:
    import scapy_local.layers.inet6
